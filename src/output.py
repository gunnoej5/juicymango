...
class Output(object):
    ...
    def send_output(self):
        ...
        def to_Sqlite3():
            ...
            hash = hashlib.md5()
            hash.update(rmNonprint(self.modname).encode('utf-8'))
            hash.update(rmNonprint(self.username).encode('utf-8'))
            hash.update(rmNonprint(self.hostname).encode('utf-8'))
            hash.update(rmNonprint(self.ircchan).encode('utf-8'))
            hash.update(rmNonprint(self.msg).encode('utf-8'))
        ...
        if output_count == 0 or check_config("OUTPUT_CONSOLE=") == "ON":
            ...
            print(f'\033[94m{rmNonprint(self.modname)}: \033[0m{str(datetime.datetime.now())}{conString}')

def print_status(module, message):
    print(f"\033[92m{module}: {message}\033[0m")

def print_error(module, message):
    print(f"\033[91m{module}: {message}\033[0m")
