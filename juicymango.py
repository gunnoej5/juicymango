...
import sys, time, sqlite3, os, logging
...
print("Website: https://github.com/gunnoej5/juicymango")
...
if check_config("OUTPUT_SQLITE3") == 'ON':
    ...
    if not os.path.isfile("users.db"):
        ...
        # Placeholder bcrypt hash for 'admin' password. Please generate a strong password and replace this hash.
        c.execute("INSERT INTO users VALUES(NULL,'admin','$2a$12$VcerjbCyiS2pJWUxWSgkU.67EPY918gKRuV4VIiano6zDMqHu4VCG','yes')")
        conn.commit()
        conn.close()
...
if mod_counter == 0:
    logging.warning(f"{module}: No modules have been configured.")
...
# Let the program continue to run until an interrupt is received.
while 1:
    try:
        time.sleep(100000)
