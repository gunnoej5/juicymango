...
def main():
	...
	cookie_secret = check_config("COOKIE_SECRET")
	
	#Route for the webroot
	@route('/')
	def mainview():
		username = request.get_cookie("loggedin", secret=cookie_secret)
		if username:
	...
	def logincheck():
		if request.forms.get("login_user") and request.forms.get("login_password"):
			...
			database = "users.db"
			conn = sqlite3.connect(database)
			stored_password_hash = conn.cursor().execute("SELECT password FROM users WHERE username = ?", (user,)).fetchone()
			conn.close()

			if stored_password_hash and bcrypt.checkpw(request.forms.get("login_password").encode('utf-8'), stored_password_hash[0].encode('utf-8')):
				response.set_cookie("loggedin", request.forms.get('login_user'), secret=cookie_secret)
	...
	def pass_set():
		username = request.get_cookie("loggedin", secret=cookie_secret)
		if username:
			current_password = request.forms.get('currentpass').encode('utf-8')
			new_password = request.forms.get('newpass').encode('utf-8')

			database = "users.db" 
			conn = sqlite3.connect(database)
			
			stored_password_hash = conn.cursor().execute("SELECT password FROM users WHERE username = ?", (username,)).fetchone()

			if stored_password_hash and bcrypt.checkpw(current_password, stored_password_hash[0].encode('utf-8')):
				new_password_hash = bcrypt.hashpw(new_password, bcrypt.gensalt()).decode('utf-8')
				conn.cursor().execute("UPDATE users SET password = ? WHERE username = ?", (new_password_hash, username))
				conn.commit()
				conn.close()
				response.delete_cookie("loggedin")
				return "1"
	...
	logging.info(f"{module}: Connect to http://{ip}:{port}")
...
if check_outputsqlitedb == "ON":
	logging.info(f"{module}: loading...")
	_thread.start_new_thread(main, ())
else:
	logging.error(f"{module}: Cannot start, no database configured.")
